...
// See VertexGen_Decay0.hh for more details
//———————————————————————//
...
VertexGen_Decay0::VertexGen_Decay0(const char *arg_dbname)
  : GLG4VertexGen(arg_dbname), fDecay0(0)
{
  fLoE=-1;
  fHiE=-1;
  fLdecay=0;
      ...
      G4double dt)
{
  ...
  G4ParticleDefinition* particleDef1 = theParticleTable->GetIon(Z1, A1, 0);
  ...
  if(A2>0){
    G4ParticleDefinition* particleDef2 = theParticleTable->GetIon(Z2, A2, 0);
    // Add the parent isotope information (stationary, no excitation)
  ...
  for (G4int j=0;j<nParticles;j++){
	  ...
	  if(time>0 && A2>0){
	    vertinfo->SetVertexParentParticle(parent2);
	  }else{
	    vertinfo->SetVertexParentParticle(parent1);	    
	  }
...
G4String VertexGen_Decay0::fCodeToName(G4int code)
{
  if (code == 1){
     ...
     Log::Die("VertexGen_Decay0: Unknown particle code " + code);
     return " ";
  }  
}
///-------------------------------------------------------------------------
void VertexGen_Decay0::SetState(G4String newValues)
{
  ...
  if (newValues.length() == 0) {
     ...
     info << "Ac228        Ar39   Ar42        As79  Bi207  Bi208  Bi210 \n" 
          << "Bi212  Bi212Po212  Bi214  Bi214Po214    C14   Ca48  Cd113 \n"
  ...
  if (fType=="2beta"){  
     ...
     try { fLdecay = DB::Get()->GetLink("Decay0", isotope);}
     catch (DBNotFoundError &e) {Log::Die("VertexGen_Decay0: can't find isotope " + isotope);}
    ...
    try { fLdecay = DB::Get()->GetLink("BackgDecay0", isotope); }
    catch (DBNotFoundError &e) {Log::Die("VertexGen_Decay0: can't find isotope " + isotope);}
    fIsotope = isotope;
    #ifdef DEBUG
     info << "VertexGen_Decay0: isotope: " << fIsotope << "\n";
...
void VertexGen_Decay0::GetParentAZ(G4int &A1, G4int &Z1, G4int &A2, G4int &Z2)
{
        ...
        // eg: Bi214Po214 -> Bi_a_214_b_Po_c_214, 
        // K40 -> K_a_40, b=c=0
        ...
        debug << fIsotope << "! " << A1 << " " << Z1 << " " << A2 << " " << Z2 << newline;   
}
} // namespace RAT
